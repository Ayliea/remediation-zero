...
"""The reviewer agent, as the deployed orchestrator delegates to it.
...
def _find_prompt(start: Path = None) -> Path:
    ...
    one of them. In the repository this file is agents/reviewer/agent.py, two
    levels below the prompts directory. Agent Engine stages each
    --extra_packages entry at /app/<basename>, so the same file is /app/reviewer/
...
def _model() -> str:
    ...
    if not model:
        raise ValueError(
            "REVIEWER_MODEL is not set. Copy .env.example to .env and set it."
        )
    return model
...
def build() -> Agent:
    ...
    return Agent(
        name=AGENT_NAME,
        model=_model(),
        description="Ratifies or rejects a triage proposal with a stated reason.",
