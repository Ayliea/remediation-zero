...
`sla_clocks` and `tickets` are derived state: chase rebuilds both from the
decisions that produced them, so clearing them costs nothing that cannot be
recomputed. Everything else is refused. `decisions` and `human_queue` are the
...
#: Derived state. Chase recomputes both from the decisions that produced them.
CLEARABLE = frozenset({"sla_clocks", "tickets"})
