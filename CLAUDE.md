...
5. **Every authoritative state-changing tool takes an idempotency key.** Derived deterministically from finding ID, action type, and cycle number. A resumed agent must never apply a ticket, nudge, or escalation transition twice. Tracker notifications are an at-least-once projection of that state: issue creation and comments with persisted, unguessable delivery markers have recovery lookups, but the GitHub API offers no atomic comment idempotency key and the narrow concurrent-post race must not be described as exactly-once.
6. **Every persisted business-time read goes through `SimClock`.** Never call `datetime.now()`, `time.time()`, or a database server timestamp to stamp a lifecycle record or make a lifecycle decision. Runtime-only concerns such as cache expiry and network timeouts may use process time and are never persisted. Every persisted lifecycle record carries both `real_ts` and `sim_ts`. `real_ts` is wall clock and is never falsified or backdated under any circumstances.
...
12. **Untrusted text passes Model Armor before reaching any reasoning context.** The implemented ingress surfaces are scanner comment fields and NVD advisory prose. Findings metadata generated by the seed script is trusted; free text carried inside it is not. Ticket replies are not ingested; if that feature is added, those replies must cross the same boundary first.
...
| Guardrails | Model Armor on all implemented untrusted ingress |
...
- Model calls retry with exponential backoff and jitter, capped at four attempts (`_with_backoff` in `tools/review_models.py`, `RetryConfig` on the graph nodes). **Tracker delivery is a coalescing, at-least-once projection**: a failure is persisted and never fails or freezes the Firestore state machine; the next cycle retries it, while a newer lifecycle event supersedes an older undelivered reminder. Issue creation and comments with persisted delivery markers are looked up before recovery delivery, but concurrent GitHub comment posts can still race because the API provides no atomic idempotency key.
